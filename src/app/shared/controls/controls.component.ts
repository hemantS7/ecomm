...
export class ControlsComponent implements OnInit {
  ...
  public decrement(count){
    if(this.count > 1){
      ...
      let obj = {
        ...
        remainingQuantity: this.count,
        total: this.count * this.product.MRP_RATE
      }
      this.changeQuantity(obj);
    }
  }
  ...
  public addToCart(product:Product){
   ...
   console.log(this.appService.Data)
    let currentProduct = this.appService.Data.cartList.filter(x=>x.ITEM_ID == product.ITEM_ID)[0];
    if(currentProduct){
      alert("here")
