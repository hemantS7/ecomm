...
export class ProductsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sub = this.activatedRoute.params.subscribe(params => {
      ...
      this.getCategories(this.catgKey);
      this.getBrands();
    });
  ...
  public getCategories(categoryName){  
    console.log(categoryName);
      this.productCategoriesService.getCategoryDetails().subscribe(data => {
        this.categories = data.result.catrgoryWiseItems;
        if(this.categories.length > 0)
        this.products = this.categories.filter(x => x.categoryName.toLowerCase() == categoryName.toLowerCase())[0].products;
      });
  }
