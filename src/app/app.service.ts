...
export class AppService {
    ...
    public updatecartInLocalstorage(){
        if(JSON.parse(localStorage.getItem('cartInfo'))){
            localStorage.removeItem("cartInfo");
        }
        localStorage.setItem('cartInfo',JSON.stringify(this.Data));
    }

    public addToCompare(product:Product){
        let message, status;
        if(this.Data.compareList.filter(item=>item.id == product.id)[0]){
            message = 'The product ' + product.ITEM_NAME + ' already added to comparison list.'; 
            status = 'error';     
        }
        else{
            this.Data.compareList.push(product);
            message = 'The product ' + product.ITEM_NAME + ' has been added to comparison list.'; 
            status = 'success';  
        }
    ...
    public addToWishList(product:Product){
        ...
        if(this.Data.wishList.filter(item=>item.id == product.id)[0]){
            message = 'The product ' + product.ITEM_NAME + ' already added to wish list.'; 
            status = 'error';     
        }
        else{
            this.Data.wishList.push(product);
            message = 'The product ' + product.ITEM_NAME + ' has been added to wish list.'; 
            status = 'success';  
        }
        this.snackBar.open(message, '×', { panelClass: [status], verticalPosition: 'top', duration: 3000 });
    } 
    ...
    public addToCart(product:Product){
        ...
        if(this.Data.cartList.filter(x=>x.ITEM_ID == product.ITEM_ID)[0]){ 
            let item = this.Data.cartList.filter(item=>item.ITEM_ID == product.ITEM_ID)[0];
        ...
        this.Data.cartList.forEach(product=>{
            this.Data.totalPrice = this.Data.totalPrice + (product.cartCount * product.MRP_RATE);
            this.Data.totalCartCount = this.Data.totalCartCount + product.cartCount;
        });

        this.updatecartInLocalstorage();
        
        message = 'The product ' + product.ITEM_NAME + ' has been added to cart.'; 
        status = 'success';          
        this.snackBar.open(message, '×', { panelClass: [status], verticalPosition: 'top', duration: 3000 });
    }
